...
@pytest.fixture
def get_token(setup_and_set_down_acts):
...
def create_and_confirm_friendship(get_token):
    client.post(endpoint,
                json=dict(friend_id=2),
                headers={"Authorization": "Bearer " + get_token.get("token")}
                )
...
def delete_friendship(get_token):
    client.request(method="DELETE", url=endpoint,
                   json={"friend_id": 2},
                   headers={"Authorization": "Bearer " + get_token.get("token")})


def test_delete_a_friend_by_id(get_token):
    client.post(endpoint,
                json=dict(friend_id=2),
                headers={"Authorization": "Bearer " + get_token.get("token")}
                )
...
def test_add_a_friend_by_id(get_token):
    ...
    delete_friendship(get_token)


def test_get_friend_list(get_token):
    client.post(endpoint,
                json=dict(friend_id=2),
                headers={"Authorization": "Bearer " + get_token.get("token")}
                )
...
def test_confirm_a_friend(get_token):
    token = create_and_confirm_friendship(get_token)
...
def test_get_friend_tasks(get_token):
    token = create_and_confirm_friendship(get_token)

    response = client.get(endpoint + "/tasks",
                          headers={"Authorization": "Bearer " + token, "friend-id": "1"})
    assert response.status_code == 200
...
def test_create_a_task_with_a_friend(get_token):
    token = create_and_confirm_friendship(get_token)
    ...
    response = client.post(endpoint + "/tasks",
                           json=task_data,
                           headers={"Authorization": "Bearer " + get_token.get("token"), "friend-id": "2"})
    assert response.status_code == 200

    tasks = client.get("/tasks", headers={"Authorization": "Bearer " + token})
